...
__all__ = ['Compose', 'Normalize', 'CenterCrop', 'RgbToGray', 'RandomCrop',
           'HorizontalFlip', 'AddNoise', 'NormalizeUtterance','Resize']
class Resize(object):
    ...
    def __call__(self, frames):
        """
        Args:
            frames (numpy.ndarray): Images to be resized.
        Returns:
            numpy.ndarray: Resized images.
        """
        t, h, w = frames.shape
        resized_frames = np.zeros((t, self.size[0], self.size[1]), dtype=frames.dtype)
        for i in range(t):
            resized_frames[i] = cv2.resize(frames[i], self.size[::-1], interpolation=cv2.INTER_LINEAR)
