...
class talkNet(nn.Module):
    ...
    def train_network(self, loader, epoch, **kwargs):
        ...
        for num, (audioFeature, visualFeature, labels) in enumerate(loader, start=1):
            self.zero_grad()
            # audioEmbed = self.model.forward_audio_frontend(audioFeature[0].cuda()) # (13,159?,128)
            visualEmbed = self.model.forward_visual_frontend(visualFeature[0].cuda())# (13?,117,128)
            audioEmbed=torch.rand_like(visualEmbed).cuda()
            audioEmbed, visualEmbed = self.model.forward_cross_attention(audioEmbed, visualEmbed)#(10?,117,128),#(10?,117,128)
            outsAV= self.model.forward_audio_visual_backend(audioEmbed, visualEmbed)  #(1170,256)
            outsA = self.model.forward_audio_backend(audioEmbed) #(1170=10*117,128)
            outsV = self.model.forward_visual_backend(visualEmbed)#(1170=10*117,128)
