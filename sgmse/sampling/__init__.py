...
def get_pc_sampler(
    ...
    """
    predictor_cls = PredictorRegistry.get_by_name(predictor_name) #ReverseDiffusionPredictor
    corrector_cls = CorrectorRegistry.get_by_name(corrector_name) #AnnealedLangevinDynamics
    ...
    def pc_sampler():
        ...
        with torch.no_grad():
            ...
            timesteps = torch.linspace(sde.T, eps, sde.N, device=y.device) #shape 30, (1->0)
...
def my_get_pc_sampler(
    predictor_name, corrector_name, sde, score_fn, y, visual_embedding=None,
    denoise=True, eps=3e-2, snr=0.1, corrector_steps=1, probability_flow: bool = False,
    intermediate=False, **kwargs
):
    """Create a Predictor-Corrector (PC) sampler.
    ...
    def pc_sampler():
        ...
        with torch.no_grad():
            xt = sde.prior_sampling(y.shape, y).to(y.device)
            timesteps = torch.linspace(sde.T, eps, sde.N, device=y.device) #shape 30, (1->0)
            for i in tqdm(range(sde.N), desc="Sampling progress"):
                t = timesteps[i]
                vec_t = torch.ones(y.shape[0], device=y.device) * t
                xt, xt_mean = corrector.update_fn(xt, vec_t, y=y, visual_embedding=visual_embedding)
                xt, xt_mean = predictor.update_fn(xt, vec_t, y=y, visual_embedding=visual_embedding)
            x_result = xt_mean if denoise else xt
            ns = sde.N * (corrector.n_steps + 1)
            return x_result, ns
...
def get_ode_sampler(
    sde, score_fn, y, visual_embedding,inverse_scaler=None,
    ...
    def denoise_update_fn(x):
        ...
        _, x = predictor.update_fn(x, vec_eps, y,visual_embedding=visual_embedding)
        return x

    def drift_fn(x, t, visual_embedding,y):
        """Get the drift function of the reverse-time SDE."""
        return rsde.sde(x, t, visual_embedding,y)[0]
    ...
    def ode_sampler(z=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            def ode_func(t, x):
                ...
                drift = drift_fn(x, vec_t, visual_embedding,y)
