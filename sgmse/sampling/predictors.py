...
class EulerMaruyamaPredictor(Predictor):
    ...
    def update_fn(self, x, t, visual_embedding,*args):
        dt = -1. / self.rsde.N
        z = torch.randn_like(x)
        f, g = self.rsde.sde(x, t, visual_embedding,*args)
        x_mean = x + f * dt
...
class ReverseDiffusionPredictor(Predictor):
    ...
    def update_fn(self, x, t, y,visual_embedding):#sdes.SDE.reverse.RSDE    
        f, g = self.rsde.discretize(x, t, y,visual_embedding) #1번.
        z = torch.randn_like(x)
        x_mean = x - f
