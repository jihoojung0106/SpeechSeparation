...
class LangevinCorrector(Corrector):
    ...
    def update_fn(self, x, t, y,visual_embedding,*args, **kwargs):
        target_snr = self.snr
        for _ in range(self.n_steps): #1
            if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                grad = self.score_fn(x, t,visual_embedding=visual_embedding,score_conditioning=kwargs["conditioning"], sde_input=y) #stochastic regeneration
            else:
                grad = self.score_fn(x, t, visual_embedding=visual_embedding)
...
class AnnealedLangevinDynamics(Corrector):
    ...
    def update_fn(self, x, t, y,visual_embedding, *args, **kwargs):
        ...
        for i in range(n_steps):
            if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                grad = self.score_fn(x, t, visual_embedding,score_conditioning=kwargs["conditioning"], sde_input=y)
            else:
