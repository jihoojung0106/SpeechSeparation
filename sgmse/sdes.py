...
class SDE(abc.ABC):
    ...
    def discretize(self, x, t, y):
        ...
        """
        dt = 1 / self.N
        drift, diffusion = self.sde(x, t, y) #3번. OUESVD의 sde함수
        f = drift * dt
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def sde(self, x, t, visual_embedding,y):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                rsde_parts = self.rsde_parts(x, t, visual_embedding,y)
                total_drift, diffusion = rsde_parts["total_drift"], rsde_parts["diffusion"]
                return total_drift, diffusion

            def rsde_parts(self, x, t, visual_embedding,y):
                sde_drift, sde_diffusion = sde_fn(x, t, y)
                score = score_model(x, t, y, visual_embedding)
            ...
            def discretize(self, x, t, y, visual_embedding):
                """discretize_fn은 SDE의 discretize"""
                f, G = discretize_fn(x, t, y) #2번. predictor update_fn의 discreitize하면 일로옴. 
                rev_f = f - G[:, None, None, None] ** 2 * score_model(x, t, y, visual_embedding) * (0.5 if self.probability_flow else 1.)
...
class OUVESDE(SDE):
    ...
    def sde(self, x, t, y): #4번.
        drift = self.theta * (y - x) #이 y는 discriminator의 결과
    ...
    def _mean(self, x0, t, y): #9번 식
        theta = self.theta # γ 논문에서의 감마
        exp_interp = torch.exp(-theta * t)[:, None, None, None] #(1,1,1,1)
        return exp_interp * x0 + (1 - exp_interp) * y #x:(1,1,256,256)
