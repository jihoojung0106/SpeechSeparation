...
class SDE(abc.ABC):
    ...
    def discretize(self, x, t, y,*args):
        ...
        """
        dt = 1 / self.N #3번. 
        drift, diffusion = self.sde(x, t, y, *args) #4번,OUVESDE.sde
        f = drift * dt
    ...
    def reverse(oself, score_model, probability_flow=False, diffusion_power_gradient=None):
        ...
        class RSDE(oself.__class__):
            ...
            def discretize(self, x, t, y,visual_embedding,*args, **kwargs):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                f, G = discretize_fn(x, t, y,*args) #2번. (OU)SDE.discretize
                if G.ndim < x.ndim:
                    G = G.view(*G.size(), *((1,)*(x.ndim - G.ndim)))
                if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                    rev_f = f - G ** 2 * score_model(x, t, visual_embedding,score_conditioning=kwargs["conditioning"], sde_input=y) * (0.5 if self.probability_flow else 1.) #SGMSE 논문 5번식
                else:
